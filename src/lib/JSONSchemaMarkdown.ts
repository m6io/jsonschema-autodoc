...
export class JSONSchemaMarkdown {
  ...
  typeNumber(name, data, level, path) {
    ...
    if (
      this.notEmpty(data.exclusiveMinimum) ||
      this.notEmpty(data.exclusiveMaximum)
    ) {
      this.indent(level)
  ...
  typeObject(name, data, level, path) {
    ...
    if (
      this.notEmpty(data.minProperties) ||
      this.notEmpty(data.maxProperties)
    ) {
      this.indent(level)
  ...
  writePattern(pattern, level) {
    if (this.notEmpty(pattern)) {
      this.writeLine(
        "The value must match this pattern: `" + pattern + "`",
        level,
      )
    }
  }
  ...
  writePropertyNames(data, level) {
    if (this.notEmpty(data) && this.notEmpty(data.pattern)) {
      this.writeLine(
        "Property names must match this pattern: `" + data.pattern + "`",
        level,
      )
    }
  }
  ...
  writeRef(ref, level) {
    if (this.notEmpty(ref)) {
      this.writeLine(
        "&#36;ref: [" + this.escapeLink(ref) + "](" + this.refLink(ref) + ")",
        level,
      )
    }
  }
